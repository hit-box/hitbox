...
fn test_successful_check_policy_non_cacheable() {
    ...
    let successful = UpstreamPolledSuccessful { adapter, result: 42 };
    let expected: CachePolicyChecked<MockAdapter<i32>, i32> = CachePolicyChecked::NonCacheable(
        CachePolicyNonCacheable { result: 42 }
    );
