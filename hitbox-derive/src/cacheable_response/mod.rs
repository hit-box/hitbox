...
use cached_type::{CachedType, CloneImpl};
...
pub(crate) fn expand(input: &DeriveInput) -> Result<TokenStream, Error> {
    ...
    if source.has_skipped_fields() {
        // Generate separate Cached type with custom Clone and trait impl
        let cached_type = CachedType::new(&source);
        let clone_impl = CloneImpl::new(&source, &cached_type);
        let trait_impl = CacheableResponseImpl::with_cached_type(&source, &cached_type);
        let generator = Generator::new(Some(&cached_type), Some(&clone_impl), &trait_impl);
        Ok(quote! { #generator })
    } else {
        // Generate simple impl where Cached = Self
        let trait_impl = CacheableResponseImpl::simple(&source);
        let generator = Generator::new(None, None, &trait_impl);
        Ok(quote! { #generator })
    }
}
