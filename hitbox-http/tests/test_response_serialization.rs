...
fn compare_responses(original: &Response<String>, restored: &Response<String>) -> Vec<String> {
...
fn assert_responses_equal(original: Response<String>, restored: Response<String>) {
    let original_body = original.body();
    let restored_body = restored.body();

    if original_body != restored_body {
        panic!(
            "Response body mismatch:\nExpected: {:?}\nGot: {:?}",
            original_body, restored_body
        );
    }
...
async fn roundtrip_test(response: Response<String>) -> Response<String> {
    ...
    let cacheable_restored = CacheableHttpResponse::<String>::from_cached(deserialized).await;
...
async fn test_basic_response() {
    ...
    assert_responses_equal(original_clone, restored);
}
...
async fn test_different_status_codes() {
    ...
    for (code, body) in test_cases {
        ...
        let restored_body = restored.into_body();
...
async fn test_different_body_types() {
    ...
    let original = Response::builder()
        ...
        .unwrap();
    let restored = roundtrip_test(original).await;
    assert_eq!(restored.into_body(), "small");
    ...
    let original = Response::builder()
        ...
        .unwrap();
    let restored = roundtrip_test(original).await;
    assert_eq!(restored.into_body(), large_body);
}
...
async fn test_binary_data() {
    ...
    let original = Response::builder()
        ...
        .body(String::from_utf8_lossy(&binary_data).to_string())
        .unwrap();
...
async fn test_many_headers() {
    ...
    let original = builder.body("many headers".to_string()).unwrap();
...
async fn test_comparison_detects_status_mismatch() {
    ...
    assert_responses_equal(original, different);
}
